...
class KafkaPacketProducer(
  ...
  val codec = if (compress) SnappyCompressionCodec.codec else NoCompressionCodec.codec
  ...
  private def kafkaMessage(message: Array[Byte], key: Array[Byte]): ProducerRecord[AnyRef, AnyRef] = {
  ...
  def send(message: String, key: String = null): Unit = send(message.getBytes("UTF8"), if (key == null) null else key.getBytes("UTF8"))

  def send(message: Array[Byte], key: Array[Byte]): Unit = {
    try {
      producer.send(kafkaMessage(message, key))
    } catch {
      case e: Exception =>
        e.printStackTrace()
        logger.error("topic:[" + topic + "], brokerlist:[" + brokerList + "], messageSize:[" + message.length + "]", e)
    }
  }
